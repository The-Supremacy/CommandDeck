...
```sh
aspire start --apphost orchestration/CommandDeck.Orchestration/CommandDeck.Orchestration.csproj --isolated
```
...
`orchestration/CommandDeck.Orchestration/Properties/launchSettings.json`;
...
`orchestration/CommandDeck.Orchestration/Realms/`. The local Keycloak
resource is exposed on port `8080`, and the Host uses
`http://localhost:8080/realms/commanddeck` as its development OIDC
authority.

The Redis resource is referenced by the Host as
`ConnectionStrings:session-tickets`. The PostgreSQL database resource is
referenced as `ConnectionStrings:commanddeck-host`.
