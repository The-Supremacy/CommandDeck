...
- Module stores depend on narrow module DbContext interfaces, not the concrete
  Host DbContext.
- Shared persistence lives in `CommandDeck.Persistence`, a Host-owned
  composition project with the concrete EF Core `CommandDeckDbContext`.
...
`CommandDeck.Host/Features/{FeatureName}`. Endpoint mapping, endpoint-local
